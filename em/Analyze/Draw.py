...
file=ROOT.TFile("final_emu_cut.root","r")

adapt=ROOT.gROOT.GetColor(12)
new_idx=ROOT.gROOT.GetListOfColors().GetSize() + 1
trans=ROOT.TColor(new_idx, adapt.GetRed(), adapt.GetGreen(),adapt.GetBlue(), "",0.5)

categories=["em_0jet_low","em_0jet_high","em_1jet_low","em_1jet_high","em_vbf_low","em_vbf_high"] 
ncat=6
...
for i in range (0,ncat):
   ...
   qqH.SetLineColor(ROOT.TColor.GetColor("#d1ca1a"))
   ggh.SetLineWidth(3)
   qqH.SetLineWidth(3)
   ...
   legende.AddEntry(qqH,"qqH 125 (X5)","l")
   legende.AddEntry(ggh,"ggh 125 (X5)","l")
   ...
   h1.Divide(hwoE)
   h3.Divide(hwoE)
   h1.GetXaxis().SetTitle("m_{#tau#tau} (GeV)")#("m_{vis} (GeV)")#(#vec{p_{T}(#mu)}+#vec{p_{T}(#tau)})/(p_{T}(#mu)+p_{T}(#tau))")
