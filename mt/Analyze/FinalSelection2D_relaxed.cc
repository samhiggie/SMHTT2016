...
cout<<"rss"<<endl;
   ...
   float bins0[] = {0,55,60,65,70,75,80,85,90,95,100,105,110,115,400};
   //float bins0[] = {0,70,80,85,90,95,100,105,110,115,120,125,130,135,140,145,150,165,180,400};
   float bins1[] = {0,70,80,90,100,110,120,130,140,150,180,300};
   float bins2[] = {0,90,105,120,135,150,200,400};

   //float bins0[] = {40,55,60,65,70,75,80,85,90,95,100,105,110,115,120,125,150};
   //float bins1[] = {40,70,80,90,100,110,120,130,140,150,180,220};
   //float bins2[] = {60,90,105,120,135,150,200};

   float bins_pth[] = {0,60,100,140,180,220,500};
   float bins_mjj[] = {300,600,900,1200,1700,3000};
   float bins_taupt[] = {30,35,40,45,50,55,300};

   //float bins_pth[] = {0,60,100,140,180,220,250};
   //float bins_mjj[] = {300,600,900,1200,1700};
   //float bins_taupt[] = {30,35,40,45,50,55};
   ...
   for (Int_t i = 0; i < nentries_wtn; i++) {
	...
	float signalRegion=(byTightIsolationMVArun2v1DBoldDMwLT_2 && iso_1<0.15 && againstElectronVLooseMVA6_2 && againstMuonTight3_2);
	float qcdRegion=(byMediumIsolationMVArun2v1DBoldDMwLT_2 && iso_1<0.30);
        float wRegion=(byMediumIsolationMVArun2v1DBoldDMwLT_2 && iso_1<0.15 && againstElectronVLooseMVA6_2 && againstMuonTight3_2);
	...
	mt=TMass_F(mymu.Pt(),met_pt2,mymu.Px(),mex,mymu.Py(),mey);

	//******************** W reweighting in high MT sideband ****************************
        if (q_1*q_2<0 && mt>80 && signalRegion) n70passOS+=poidsW*aweight;

	//************************* Pzeta variable ***********************
	float norm_zeta=norm_F(mytau.Px()/mytau.Pt()+mymu.Px()/mymu.Pt(),mytau.Py()/mytau.Pt()+mymu.Py()/mymu.Pt());
	float x_zeta= (mytau.Px()/mytau.Pt()+mymu.Px()/mymu.Pt())/norm_zeta;
	float y_zeta= (mytau.Py()/mytau.Pt()+mymu.Py()/mymu.Pt())/norm_zeta;
	float p_zeta_mis=mex*x_zeta+mey*y_zeta;
	float pzeta_vis=(mytau.Px()+mymu.Px())*x_zeta+(mytau.Py()+mymu.Py())*y_zeta;
    ...
    TDirectory *OS1jet =fout->mkdir("mt_1jet");
    OS1jet->cd();
