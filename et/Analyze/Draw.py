...
file=ROOT.TFile("final_etau_cut.root","r")

adapt=ROOT.gROOT.GetColor(12)
new_idx=ROOT.gROOT.GetListOfColors().GetSize() + 1
trans=ROOT.TColor(new_idx, adapt.GetRed(), adapt.GetGreen(),adapt.GetBlue(), "",0.5)

categories=["et_0jet_low","et_0jet_high","et_1jet_low","et_1jet_high","et_vbf_low","et_vbf_high"] 
ncat=6
...
for i in range (0,ncat):
   ...
   DYB=file.Get(categories[i]).Get("ZLL")
   ...
   errorBandDYB=file.Get(categories[i]).Get("ZLL").Clone()
   ...
   h3.Divide(hwoE)
   if (i==0 or i==1):
      h1.GetXaxis().SetTitle("m_{vis} (GeV)")#(#vec{p_{T}(#mu)}+#vec{p_{T}(#tau)})/(p_{T}(#mu)+p_{T}(#tau))")
   else: 
