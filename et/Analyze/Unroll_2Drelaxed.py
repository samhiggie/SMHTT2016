...
file=ROOT.TFile("final_etau_2D.root","r")
file1D=ROOT.TFile("final_etau_1D.root","recreate")

adapt=ROOT.gROOT.GetColor(12)
new_idx=ROOT.gROOT.GetListOfColors().GetSize() + 1
trans=ROOT.TColor(new_idx, adapt.GetRed(), adapt.GetGreen(),adapt.GetBlue(), "",0.5)

categories=["et_0jet","et_1jet","et_vbf"] 
cat=["et_0jet","et_1jet","et_vbf"]
ncat=3
...
for i in range (0,ncat):
   ...
   for k in range(1,Data.GetSize()-1):
	...
	if (s/((s+b)**0.5) > 0.2):
            Data.SetBinContent(k,10000000)
            Data.SetBinError(k,10000000)
   Data.Draw("e")
   stack.Draw("histsame")
   ...
   if i==0: 
    categ.AddText("e#tau_{h}, no jet")
   elif i==1:
    categ.AddText("e#tau_{h}, boosted")
   elif i==2:
    categ.AddText("e#tau_{h}, VBF")
   categ.Draw("same")
   ...
   h1.GetXaxis().SetTitleOffset(0.56)
